<script lang="ts">
	...
	async function handleExportCsv() {
		const path = await save({ title: 'Export CSV', filters: [{ name: 'CSV', extensions: ['csv'] }] });
		if (!path) return;
		try {
	...
	async function handleExportJson() {
		const path = await save({ title: 'Export BOM JSON', filters: [{ name: 'JSON', extensions: ['json'] }] });
		if (!path) return;
		try {
			await api.exportBomJson(path);
			showToast('BOM JSON exported', 'success');
		} catch (e) {
			showToast(`Error: ${e}`, 'error');
		}
	}
