...
// Mock the api module so updateProject doesn't hit a real backend
vi.mock('$lib/api', () => ({
...
describe('history store', () => {
	...
	describe('undo', () => {
		...
		it('syncs restored state to backend', async () => {
			...
			(updateProject as ReturnType<typeof vi.fn>).mockClear();
	...
	describe('edge cases', () => {
		...
		it('handles backend sync error gracefully', async () => {
			const { updateProject } = await import('$lib/api');
			(updateProject as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('Backend down'));
